...
\author{
  Lin Wenqi \quad Hu Haozhan \quad Wang Zhaohe
}
\date{\today}
...
\begin{enumerate}[nosep]
  ...
  \item A \textbf{two-stage clustering workflow} that groups semantically similar issues within each dimension-sentiment bucket and merges cross-batch redundancies, with label standardization.
...
Listing~\ref{lst:scraping} shows a simplified version of the scraping loop.

\begin{lstlisting}[language=Python, caption={Simplified comment scraping loop (from \texttt{comments.py}).}, label=lst:scraping]
...
After classification, we group related pain points into coherent topic clusters. This is non-trivial because multiple comments may describe the same issue using superficially different language. For example, ``连朋友资料都没，这算什么社交软件'' and ``好友来源、好友共同群聊、添加好友时间等信息都没有显示'' use entirely different wording but both complain about the same missing feature: friend profile information. A statistical clustering method based on word embeddings would likely separate them into unrelated clusters due to near-zero lexical overlap; our solution uses the LLM directly as the clustering engine, which can reason that both comments target the same root cause---insufficient friend information display.

\paragraph{Clustering Rules.} The System Prompt injects explicit rules guiding the LLM: merge items describing the same feature with different wording (e.g., synonyms for ``light sensor''), ignore superficial differences such as device model or OS version when the root cause is identical, split comments that contain multiple independent issues, and guarantee that every input item appears in exactly one cluster. Each output cluster includes a \texttt{cluster\_name} (4--12 Chinese characters) and a \texttt{canonical\_issue} (a one-sentence description of the root cause).

\paragraph{Two-Stage Pipeline.} Clustering proceeds in two stages:

\begin{enumerate}[nosep]
  \item \textbf{Stage~1 --- Intra-bucket clustering.} Classified items are partitioned into up to 21 homogeneous (dimension, sentiment) buckets. Within each bucket, items are deduplicated and sent to the LLM in batches of up to 18. The LLM groups them into root-cause clusters, each containing a \texttt{cluster\_name} (4--12 Chinese characters), a \texttt{canonical\_issue} (one-sentence root-cause description), \texttt{size} (number of member items), and \texttt{member\_ids} (references back to all source comment IDs).
  \item \textbf{Stage~2 --- Intra-bucket refinement.} Stage~1 splits large buckets into independent batches, which may independently produce semantically overlapping clusters. Stage~2 collects all clusters within each bucket and re-feeds them to the LLM for cross-batch deduplication and label standardization, producing a consolidated set of professionally named clusters with merged member assignments.
\end{enumerate}

\paragraph{Error Recovery.} To handle non-deterministic LLM outputs, a three-tier repair pipeline is applied: automatic deduplication removes duplicate item assignments and hallucinated IDs; token-overlap reassignment matches unassigned items to existing clusters via word overlap; and a secondary LLM repair call is invoked as a last resort. Any item that cannot be assigned is placed into a singleton cluster to prevent data loss. The pipeline supports \texttt{--resume} to persist intermediate results after each batch and continue from interruptions.
...
During clustering, identical issues from different source comments are deduplicated by normalizing text (lowercasing, whitespace collapsing, zero-width character stripping) and comparing the (dimension, sentiment, normalized pain point) triple, preventing semantically identical items from being counted multiple times.
...
project/
  ...
  scripts/
    ...
    refine_and_merge_topics.py # Stage-2 merging \& label standardization
...
\subsection{Classification and Taxonomy Design}

The seven-dimension taxonomy was designed to balance granularity with actionability:
...
Section~3.3 describes our final LLM-as-clusterer design. This section discusses the design rationale behind that choice.

Our initial clustering prototype followed a conventional NLP workflow: pain-point texts were embedded with Sentence-BERT~\cite{reimers19}, clustered via KMeans or DBSCAN, and each resulting cluster was summarized by the LLM into a label. We observed two fundamental problems with this approach:

\begin{enumerate}[nosep]
  \item \textbf{Surface-form blindness.} Revisiting the friend-information example from Section~3.3: the phrases ``连朋友资料都没'' and ``好友来源、好友共同群聊等都没有'' share almost no common words, so their embedding vectors would be far apart and assigned to different clusters---despite both targeting the identical root cause of insufficient friend information display. Word embeddings capture lexical similarity but cannot detect root-cause equivalence.
  \item \textbf{Short-text noise.} Most pain-point items are 10--30 Chinese characters, yielding sparse, high-dimensional vectors that make unsupervised clustering boundaries unreliable.
\end{enumerate}

These observations motivated the LLM-as-clusterer approach, where the LLM directly partitions items without an intermediate embedding step. The System Prompt injects domain rules---merge synonyms, ignore device-model differences, split multi-issue items, guarantee coverage---which allow the model to reason about root causes rather than lexical surface forms.

The two-stage decomposition addresses a fundamental tension: the full set of classified items exceeds a single LLM prompt, but splitting items arbitrarily would break semantic dependencies across batches. We resolve this by first partitioning items by (dimension, sentiment), so each batch is semantically homogeneous (e.g., all complaints about UI and interaction). This focus increases the chance that related items co-occur in the same batch and are correctly merged. However, batching introduces a side effect---different batches within the same bucket may independently discover the same root cause. Stage~2 reconciles this by re-feeding the much smaller set of cluster summaries to the LLM in one call for cross-batch deduplication and label standardization. In essence, the two-stage design decomposes a large clustering problem into focused sub-problems with a lightweight reconciliation step.

Our qualitative inspection indicates that this approach produces substantially more coherent groupings than the embedding-based alternative.

\subsection{Robustness Mechanisms}

The LLM-as-clusterer approach introduces a practical challenge: LLM outputs are non-deterministic. Across our dataset, we observed recurring failure modes---duplicate item assignments across multiple clusters, missing items not assigned to any cluster, and hallucinated item IDs. The three-tier repair pipeline described in Section~3.3 was developed to handle these issues progressively:

\begin{enumerate}[nosep]
  \item \textbf{Automatic deduplication} resolves duplicate assignments by retaining the first occurrence.
  \item \textbf{Token-overlap reassignment} matches unassigned items to existing clusters by computing simple word overlap between the item text and cluster descriptors.
  \item \textbf{LLM re-repair} is invoked only as a last resort, re-feeding the original output with validation errors to the model.
\end{enumerate}

This layered design reflects a practical principle when building LLM-integrated pipelines: deterministic programmatic repair should be exhausted before resorting to additional LLM calls, which are both expensive and themselves fallible. The singleton fallback ensures that no item is silently discarded: even if all repair steps fail, the item remains visible in the dashboard as a standalone entry.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{No ground-truth evaluation.} We did not conduct a formal human-annotated evaluation of classification or clustering accuracy. Future work should include a manually labeled subset to enable quantitative measurement of precision and recall.
...
\bibitem[Reimers and Gurevych(2019)]{reimers19}
N.~Reimers and I.~Gurevych.
\newblock Sentence-BERT: Sentence embeddings using Siamese BERT-networks.
\newblock In \textit{Proc. Conference on Empirical Methods in Natural Language Processing (EMNLP)}, 2019.
