...
\author{Lin Wenqi \quad Hu Haozhan \quad Wang Chaohe}
...
\begin{frame}{Contributions}
  \begin{benumerate}
    ...
    \item \textbf{Two-stage clustering} --- intra-bucket grouping + cross-bucket merging
...
\begin{frame}{Stage 2: LLM-Based Classification}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{Sentiment (3 labels)}
      \begin{itemize}
        \item \textcolor{green!60!black}{Positive}
        \item \textcolor{red!60!black}{Negative}
        \item \textcolor{gray}{Neutral}
      \end{itemize}

      \vspace{0.4cm}
      \textbf{7 Core Dimensions}
      \begin{benumerate}\footnotesize
        \item Functional Experience
        \item System \& Device Compatibility
    ...
    \begin{column}{0.52\textwidth}
      ...
      \begin{itemize}
        \item \textbf{JSON mode} --- structured output enforcement
        \item \textbf{Batch processing} --- 10 comments/batch
        \item \textbf{3-tier error recovery}:\\
              \scriptsize 1.~Extract JSON from markdown fences\\
              \scriptsize 2.~Heuristic brace extraction\\
              \scriptsize 3.~Secondary LLM repair call
        \item \textbf{Actionable Suggestion} for every negative issue
        \item \textbf{Idempotent IDs} --- traceable to raw comments
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Stage 3: Two-Stage Topic Clustering}
  \begin{block}{Stage 1 --- Intra-bucket Clustering}
    \begin{itemize}\footnotesize
      \item Within each (dimension, sentiment) bucket, LLM groups items by root cause
      \item Up to 18 items per batch $\rightarrow$ canonical issue description + display name
    \end{itemize}
  \end{block}

  \begin{block}{Stage 2 --- Cross-bucket Merging}
    \begin{itemize}\footnotesize
      \item Identifies semantically duplicate topics across dimension-sentiment buckets
      \item Merged clusters receive standardized topic names
    \end{itemize}
  \end{block}

  \begin{block}{Robustness Mechanisms}
    \begin{itemize}\footnotesize
      \item \textbf{Auto-repair:} malformed outputs $\rightarrow$ repair pass $\rightarrow$ singletons
      \item \textbf{Deduplication:} normalize text + compare (dimension, sentiment, pain\_point) triples
    \end{itemize}
  \end{block}
\end{frame}
    ...
    \begin{column}{0.5\textwidth}
      \begin{figure}
        ...
        \fbox{\begin{minipage}{0.9\textwidth}\centering
          ...
          sentiment donut chart + dimension bar chart
          \vspace{2cm}
        \end{minipage}}
    ...
    \begin{column}{0.5\textwidth}
      \begin{figure}
        ...
        \fbox{\begin{minipage}{0.9\textwidth}\centering
          ...
          dimension-filtered pain points\\with actionable suggestions
          \vspace{2cm}
        \end{minipage}}
    ...
    \begin{column}{0.5\textwidth}
      \begin{figure}
        ...
        \fbox{\begin{minipage}{0.9\textwidth}\centering
          ...
          topic clusters by dimension $\times$ sentiment
          \vspace{2cm}
        \end{minipage}}
...
\begin{frame}{Evaluation \& Discussion}
  ...
  \begin{block}{Clustering}
    \begin{itemize}
      \item Scattered comments on the same issue $\rightarrow$ \textbf{one root-cause cluster}
      \item Auto-repair ensures malformed outputs are never silently dropped
    \end{itemize}
  \end{block}
\end{frame}
